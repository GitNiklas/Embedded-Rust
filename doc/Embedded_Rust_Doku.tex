...
Um die Entwicklungsumgebung einzurichten, sollte eine normale Rust-Installation vorhanden sein. Zur Installation siehe \href{https://www.rust-lang.org/en-US/install.html}{www.rust-lang.org/en-US/install.html}. Außerdem sollte das alternative Rust-Buildsystem xargo installiert werden: \mintinline{bash}|cargo install xargo|.
...
Damit die in \mintinline{bash}|/opt/avr-rust| installierte AVR-Toolchain genutzt wird, sollte folgender Befehl genutzt werden:
\begin{minted}[linenos=false]{bash}
...
Da unter Linux zum Verwenden des AVR-Programmers standardmäßig Root-Rechte erforderlich sind, kann folgende Zeile in der Datei \mintinline{bash}|/etc/udev/rules.d/99_usbprog.rules| hinzugefügt werden, um die Programmierung auch für normale Benutzer zu erlauben:
